...
struct State {
    ...
    void dropCurrent(){ files.removeAt(i); if(i >= files.size()) i = qMax(0, i-1); }
...
public:
    ...
    void next(){ if (s.i < s.files.size()-1){ ++s.i; showImg(); } }
    void prev(){ if (s.i > 0){ --s.i; showImg(); } }

    void renameCurrent(){
        QFileInfo fi(s.cur());
        bool ok = false;
        const QString base = fi.completeBaseName();
        QString nn = QInputDialog::getText(this, "Rename",
                                           base, QLineEdit::Normal, base, &ok);
        if (!(ok && !nn.isEmpty())) { setFocus(Qt::OtherFocusReason); return; }
        ...
        if (QFile::exists(np)) {
            if (QMessageBox::question(this, "Overwrite?",
                    QFileInfo(np).fileName() + " exists. Overwrite?") != QMessageBox::Yes) {
                setFocus(Qt::OtherFocusReason); return;
            }
            QFile::remove(np);
        }
    ...
    void deleteCurrent(){ // permanent delete
    ...
    void moveCurrent(){
        ...
        QString choice = QInputDialog::getItem(this, "Move to directory",
                                               "Destination directory:",
                                               subdirs, 0, /*editable*/true, &ok);
        if (!(ok && !choice.trimmed().isEmpty())) { setFocus(Qt::OtherFocusReason); return; }
        ...
        if (QFile::exists(target)) {
            if (QMessageBox::question(this, "Overwrite?",
                    QFileInfo(target).fileName() + " exists. Overwrite?") != QMessageBox::Yes) {
                setFocus(Qt::OtherFocusReason); return;
            }
            QFile::remove(target);
        }
    ...
    // ----- events -----
    void resizeEvent(QResizeEvent*) override { if (!s.empty()) showImg(); }
    void keyPressEvent(QKeyEvent *e) override {
        switch (e->key()){
            case Qt::Key_Right: case Qt::Key_L: next(); break;
            case Qt::Key_Left:  case Qt::Key_H: prev(); break;
            case Qt::Key_R:     renameCurrent(); break;
            case Qt::Key_M:     moveCurrent(); break;
            case Qt::Key_Delete:deleteCurrent(); break;
            case Qt::Key_Q:     close(); break;
            default: QLabel::keyPressEvent(e);
        }
    }
};
...
int main(int argc, char **argv){
    ...
    QStringList args = app.arguments(); args.removeFirst(); // drop programnavn
    if (args.isEmpty()){
        fputs("USAGE: memeview FILE [FILE...]\n", stderr);
        return 2;
    }
    // Shell ekspanderer ~, men la oss være hyggelige hvis noen siterer det
    for (QString &a : args) if (a.startsWith("~")) a.replace(0, 1, QDir::homePath());
